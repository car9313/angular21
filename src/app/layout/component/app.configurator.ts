...
import { Component, computed, inject, PLATFORM_ID, signal, ChangeDetectionStrategy, OnInit } from '@angular/core';
...
export class AppConfigurator implements OnInit {
    ...
    onPresetChange(event: string) {
        ...
        const palette = this.surfaces.find((s) => s.name === this.selectedSurfaceColor())?.palette ?? this.surfaces[0]?.palette;
        if (!palette) {
            return;
        }
